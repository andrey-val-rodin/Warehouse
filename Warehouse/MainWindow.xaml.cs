...
namespace Warehouse
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private static GoogleSheetsProvider GoogleSheetsProvider { get; } = ServiceProvider.GetService<GoogleSheetsProvider>();
        ...
        private void NewFabricationButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (fabrication != null)
            {
                ...
                Task.Run(async () => await GoogleSheetsProvider.AppendFabricationAsync(fabrication));
            }
        }
                ...
                if (changedFabrication.Status != fabrication.Status)
                {
                    switch (changedFabrication.Status)
                    {
                        case FabricationStatus.Closed:
                            ...
                            Model.FabricationViewModel.Update();
                            Task.Run(async () => await GoogleSheetsProvider.AppendFabricationAsync(changedFabrication));
                            break;
